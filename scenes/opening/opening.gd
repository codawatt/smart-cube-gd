...
func _add_textures_to_container(textures : Array[Texture2D]) -> void:
	for texture in textures:
		...
		%ImageContainer.call_deferred("add_child", texture_rect)
...
func _hide_previous_image() -> void:
	...
	if %ImageContainer.get_child_count() == 0:
		return
	var current_image = %ImageContainer.get_child(next_image_index - 1)
	if current_image:
		current_image.modulate.a = 0.0

func _show_next_image(animated : bool = true) -> void:
	_hide_previous_image()
	if next_image_index >= %ImageContainer.get_child_count():
		if animated:
			_transition_out()
		else:
			_load_next_scene()
		return
	var texture_rect = %ImageContainer.get_child(next_image_index)
	if animated: 
		tween = create_tween()
